...
const loginUser = async (payload) => {
    ...
    if (isUserExist.password &&
        ...
        throw new ApiError_1.default(http_status_codes_1.StatusCodes.UNAUTHORIZED, "Invalid Credential");
    }
    //create access token & refresh token
    const { email: userEmail, role, password: userPass } = isUserExist;
    const accessToken = jwtHelpers_1.jwtHelpers.createToken({ userEmail, role, userPass }, config_1.default.jwt.secret, config_1.default.jwt.expires_in);
    const refreshToken = jwtHelpers_1.jwtHelpers.createToken({ userEmail, role, userPass }, config_1.default.jwt.refresh_secret, config_1.default.jwt.refresh_expires_in);
    return {
        accessToken,
        refreshToken,
    };
